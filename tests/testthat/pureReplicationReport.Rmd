...
```{r}
mocapGrip:::CoefficientPlot(list(params$data[["action"]]$models$bestModel[[1]]$modelObject))
```
...
```{r}
ggplot(mocapGrip:::pred(params$data[["action"]]$models$bestModel[[1]]$modelObject)) + aes(x=stickcmScaled+8, y=maxGrip, ymin=plo, ymax=phi, group=fins, color=fins) + geom_pointrange(position = position_dodge(width=0.5)) + labs(title = "Model predictions for maximum grip", x="stick size (in cm)", y="maximum grip aperture (in mm)")
```

### Full model output

```{r, fig.keep='none', results="asis", message=FALSE}
texreg::htmlreg(list(params$data[["action"]]$models$bestModel[[1]]$modelObject),
...
mocapGrip:::ggCaterpillar(lme4::ranef(params$data[["action"]]$models$bestModel[[1]]$modelObject, condVar = TRUE))
```
## Size estimation
...
```{r}
mocapGrip:::CoefficientPlot(list(params$data[["estimation"]]$models$bestModel[[1]]$modelObject))
```
...
```{r}
ggplot(mocapGrip:::pred(params$data[["estimation"]]$models$bestModel[[1]]$modelObject)) + aes(x=stickcmScaled+8, y=meanGrip, ymin=plo, ymax=phi, group=fins, color=fins) + geom_pointrange(position = position_dodge(width=0.5)) + labs(title = "Model predictions for maximum grip", x="stick size (in cm)", y="mean grip aperture (in mm)")
```

### Full model output

```{r, fig.keep='none', results="asis", message=FALSE}
texreg::htmlreg(list(params$data[["estimation"]]$models$bestModel[[1]]$modelObject),
        method = "boot", # only needed for the overriding of pvalues
        use.se=TRUE, # only needed for the overriding of pvalues
        #        override.pval = list(lengthLMscaleSum$coefficients[,"Pr(>|t|)"]),
        float.pos = "p!",
...
```{r, results="hide", message=FALSE}
mocapGrip:::ggCaterpillar(lme4::ranef(params$data[["estimation"]]$models$bestModel[[1]]$modelObject, condVar = TRUE))
```
