...
There are varying intercepts by subject, and the effects of all the predictors  are also allowed to vary by subject (these are the mixed effects). The full formula^[See [the lme4 documentation](https://cran.r-project.org/web/packages/lme4/vignettes/lmer.pdf) (especially section 2) for explanation of the formula format.] for this model is:
...
## Maximum grip aperture during gesture

The maximum grip aperture is the maximum distance between the markers on the thumb and index finger during the period between when the hand left the table and when it looked like it was picking up the stick during gesture.
...
* the size of the stick in centimeters as a categorical variable (factor), with seven as the reference level.
* the configuration of the fins (closed, none, open; where closed is the reference level)
* the interaction between the size of the stick (as a categorical variable) and configuration of fins

There are varying intercepts by subject, and the effects of all the predictors (including interactions) are also allowed to vary by subject (these are the mixed effects). The full formula^[See [the lme4 documentation](https://cran.r-project.org/web/packages/lme4/vignettes/lmer.pdf) (especially section 2) for explanation of the formula format.] for this model is:

maxGrip ~ stick * fins + (1 + stick * fins | obsisSubj)
...
```{r}
mocapGrip:::CoefficientPlot(list(params$data[["gestMaxGrip"]]$analyses[["maxGrip.stickAsCategorical"]]$bestModel[[1]]$modelObject))
```
...
```{r}
ggplot(mocapGrip:::pred(params$data[["gestMaxGrip"]]$analyses[["maxGrip.stickAsCategorical"]]$bestModel[[1]]$modelObject)) + aes(x=stick, y=maxGrip, ymin=plo, ymax=phi, group=fins, color=fins) + geom_pointrange(position = position_dodge(width=0.5)) + labs(title = "Model predictions for grip", x="stick size (in cm)", y="the maximum grip aperture (in mm)")
```

### Full model output

```{r, fig.keep='none', results="asis", message=FALSE}
texreg::htmlreg(list(params$data[["gestMaxGrip"]]$analyses[["maxGrip.stickAsCategorical"]]$bestModel[[1]]$modelObject),
        method = "boot", # only needed for the overriding of pvalues
        use.se=TRUE, # only needed for the overriding of pvalues
        #        override.pval = list(lengthLMscaleSum$coefficients[,"Pr(>|t|)"]),
        float.pos = "p!",
...
```{r, results="hide", message=FALSE}
mocapGrip:::ggCaterpillar(lme4::ranef(params$data[["gestMaxGrip"]]$analyses[["maxGrip.stickAsCategorical"]]$bestModel[[1]]$modelObject, condVar = TRUE))
```
...
```{r}
ggplot(params$data[["gestMaxGrip"]]$data) + aes(x=stick, y=maxGrip, group=fins, color=fins) + geom_point(position = position_dodge(width=0.5), alpha = 0.5) + labs(title = "Raw data for  grip", x="stick size (in cm)", y="the maximum grip aperture (in mm)")
```
...
```{r, results='asis'}
knitr::kable(
  params$data[["gestMaxGrip"]]$data %>% dplyr::group_by(stick, fins) %>% dplyr::summarize(n=n(), subjs=length(unique(obsisSubj)), meanPerSubj=n/subjs)
)

if("gestMaxGrip" %in% c("gestMove")){
  cat("#### Observations with gripType included *this is temporary*  \n The vast majority of the movements are open, with very few being open/closed. Currently, all of these (including the closed grips) are included in the analysis. We probably should exclude the closed, and possibly even the open.closed ones in the future. Thoughts?")

knitr::kable(
  params$data[["gestMaxGrip"]]$data %>% dplyr::group_by(stick, fins, gripType) %>% dplyr::summarize(n=n(), subjs=length(unique(obsisSubj)), meanPerSubj=n/subjs)
)
}
```

There were `r length(params$data[["gestMaxGrip"]]$warnings)` trials that were excluded because there was occlusion.

```{r, results='asis'}
if(length(params$data[["gestMaxGrip"]]$warnings)>0){
  # don't print anythign if there are no occlusions.
  cat(paste0("  \n* ", params$data[["gestMaxGrip"]]$warnings, collapse = ""))
}
```


## Mean grip aperture during gesture (movement)

The mean and median distance between the markers on the thumb and index finger during the period between when the hand was moving during gesture.
...
meanGrip ~ stick * fins + (1 + stick + fins | obsisSubj)
...
```{r}
mocapGrip:::CoefficientPlot(list(params$data[["gestMove"]]$analyses[["meanGrip.stickAsCategorical"]]$bestModel[[1]]$modelObject))
```
...
```{r}
ggplot(mocapGrip:::pred(params$data[["gestMove"]]$analyses[["meanGrip.stickAsCategorical"]]$bestModel[[1]]$modelObject)) + aes(x=stick, y=meanGrip, ymin=plo, ymax=phi, group=fins, color=fins) + geom_pointrange(position = position_dodge(width=0.5)) + labs(title = "Model predictions for grip", x="stick size (in cm)", y="the mean grip aperture (in mm)")
```

### Full model output

```{r, fig.keep='none', results="asis", message=FALSE}
texreg::htmlreg(list(params$data[["gestMove"]]$analyses[["meanGrip.stickAsCategorical"]]$bestModel[[1]]$modelObject),
        method = "boot", # only needed for the overriding of pvalues
        use.se=TRUE, # only needed for the overriding of pvalues
        #        override.pval = list(lengthLMscaleSum$coefficients[,"Pr(>|t|)"]),
        float.pos = "p!",
...
```{r, results="hide", message=FALSE}
mocapGrip:::ggCaterpillar(lme4::ranef(params$data[["gestMove"]]$analyses[["meanGrip.stickAsCategorical"]]$bestModel[[1]]$modelObject, condVar = TRUE))
```
...
```{r}
ggplot(params$data[["gestMove"]]$data) + aes(x=stick, y=meanGrip, group=fins, color=fins) + geom_point(position = position_dodge(width=0.5), alpha = 0.5) + labs(title = "Raw data for  grip", x="stick size (in cm)", y="the mean grip aperture (in mm)")
```
...
```{r, results='asis'}
knitr::kable(
  params$data[["gestMove"]]$data %>% dplyr::group_by(stick, fins) %>% dplyr::summarize(n=n(), subjs=length(unique(obsisSubj)), meanPerSubj=n/subjs)
)

if("gestMove" %in% c("gestMove")){
  cat("#### Observations with gripType included *this is temporary*  \n The vast majority of the movements are open, with very few being open/closed. Currently, all of these (including the closed grips) are included in the analysis. We probably should exclude the closed, and possibly even the open.closed ones in the future. Thoughts?")

knitr::kable(
  params$data[["gestMove"]]$data %>% dplyr::group_by(stick, fins, gripType) %>% dplyr::summarize(n=n(), subjs=length(unique(obsisSubj)), meanPerSubj=n/subjs)
)
}
```

There were `r length(params$data[["gestMove"]]$warnings)` trials that were excluded because there was occlusion.

```{r, results='asis'}
if(length(params$data[["gestMove"]]$warnings)>0){
  # don't print anythign if there are no occlusions.
  cat(paste0("  \n* ", params$data[["gestMove"]]$warnings, collapse = ""))
}
```
