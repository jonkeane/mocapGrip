...
1. **Process the mocap data and move data** The motion capture data needs to be processed within *Obsis*, and then that proceessed data (especially the *Filtered Markers.txt*, which is where `mocapGrip` extarcts the mocap data from) as well as the (clipped) videos are uploaded to the server in the folder structure that is described in the `makeElanFiles(...)` section below.
