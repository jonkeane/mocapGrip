...
* `ceilingFinder` find the mean and the median of a grip that is in a band at the top of the range. How large the band is is defined by the `bandWidth` option below. The data also includes a measure of how during the period the grip is within this band (column name: `durInBand`).
options:  
    * `percOcclusion` the maximum allowable occlusion as a percentage (in decimal notation). Default: 0.05 (meaning 5%)
